...
public class LockTest {
	...
	public void testSignatureValue() {
		...
		String lockExpiryDateRepresentation = format.format(lock1
				.getExpiryDate());
		String dataToSign = subject + "_!_" + owner + "_!_"
				+ lockExpiryDateRepresentation;

		Long expectedSignature = SignatureHelper.signatureOf(dataToSign);

		assertEquals(expectedSignature, lock1.getSignature());
		
		String dataToSign2 = subject+"1"+"_!_" + owner + "_!_"
				+ lockExpiryDateRepresentation;
		
		Long notExpectedSignature = SignatureHelper.signatureOf(dataToSign2);
		
		Assert.assertFalse(notExpectedSignature.equals(lock1.getSignature()));
	}
